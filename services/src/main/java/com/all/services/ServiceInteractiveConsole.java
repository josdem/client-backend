...
public final class ServiceInteractiveConsole {

	private static final Log LOG = LogFactory.getLog(ServiceInteractiveConsole.class);

	private ServiceInteractiveConsole() {
	}
		...
		while (true) {
			...
			try {
				if (line.indexOf("hostname") >= 0) {
					hostname = line.substring(line.indexOf("hostname") + 8).trim();
					...
					execute(hostname, password, line.trim());
				}
			} catch (Exception e) {
				LOG.error(e, e);
			}
	...
	private static void execute(String hostname, String password, String command) throws IOException,
