...
import static com.all.shared.messages.SignUpResult.OK;
import static com.all.shared.messages.SignUpResult.SERVER_ERROR;
import static com.all.shared.messages.SignUpResult.USER_ALREADY_REGISTERED;
...
import com.all.backend.web.services.StatsService;
...
public class LoginServerController {
	...
	private static final String ALL_BACKEND_CONFIRM_URL = "http://all.com/confirmEmail";
	...
	public String resetRequest(@RequestBody String mail, HttpServletRequest request) {
		...
		if (resultAsarray.length == 2) {
			...
			BackendServiceHelper.queue(emailService, BackendConstants.SEND_RESET_PASSWORD_EMAIL_REQUEST_TYPE, pendingEmail);
		} else {
	...
	public String login(@RequestBody String jsonLogin, HttpServletRequest request) {
		...
		String pwd = loginCommand.getEncryptedPwd() == null || loginCommand.getEncryptedPwd().length() == 0 ? loginCommand
				.getPassword() : loginCommand.getEncryptedPwd();
	...
	private void saveLoginStats(LoginCommand loginCommand) {
		UserSpecs userSpecs = new UserSpecs();
		userSpecs.setEmail(loginCommand.getEmail());
		userSpecs.setJvm(loginCommand.getJvm());
		userSpecs.setOs(loginCommand.getOs());
		userSpecs.setOsVersion(loginCommand.getOsVersion());
		userSpecs.setPublicIp(loginCommand.getPublicIp());
		userSpecs.setScreenSize(loginCommand.getScreenSize());
		userSpecs.setTimezone(loginCommand.getTimezone());
		userSpecs.setVersion(loginCommand.getVersion());
		UserActionStat loginStat = new UserActionStat();
		loginStat.setAction(UserActions.AllNetwork.LOGIN);
		loginStat.setEmail(loginCommand.getEmail());
		loginStat.setTimes(1);
		List<AllStat> loginStats = new ArrayList<AllStat>();
		loginStats.add(userSpecs);
		loginStats.add(loginStat);
		statsService.saveStats(loginStats);
	}

	@RequestMapping(method = PUT, value = "/aboutus")
	@ResponseBody
	public void updateAboutUs(@RequestBody String indexAboutUs) {
		log.info("\nACTION:UpdateAboutUs");
		int aboutUsId = Integer.valueOf(indexAboutUs);
		userService.updateAboutUs(aboutUsId);
	}
