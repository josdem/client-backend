...
import com.all.mobile.web.model.MobileTrack;
import com.all.mobile.web.services.SearchService.FullTextSessionFactory;
...
public class TestSearchService {
	...
	public void shouldFindTracksByKeywords() throws Exception {
		...
		verify(hibernate).executeFind(callbackCaptor.capture());
		HibernateCallback<List<MobileTrack>> callback = (HibernateCallback<List<MobileTrack>>) callbackCaptor.getValue();
		...
		List<MobileTrack> resultList = new ArrayList<MobileTrack>();
		MobileTrack cachedTrack1 = new MobileTrack();
		MobileTrack cachedTrack2 = new MobileTrack();
		MobileTrack cachedTrack3 = new MobileTrack();
		resultList.add(cachedTrack1);
		resultList.add(cachedTrack2);
		resultList.add(cachedTrack3);
		...
		when(queryContextBuilder.forEntity(MobileTrack.class)).thenReturn(entitycontext);
		...
		when(fullTextSession.createFullTextQuery(luceneQuery, MobileTrack.class)).thenReturn(fullTextQuery);
		when(fullTextQuery.list()).thenReturn(resultList);

		List<MobileTrack> doInHibernateResult = callback.doInHibernate(session);

		assertEquals(cachedTrack1, doInHibernateResult.get(0));
		assertEquals(cachedTrack2, doInHibernateResult.get(1));
		assertEquals(cachedTrack3, doInHibernateResult.get(2));
		verify(fullTextQuery).setMaxResults(50);
	...
	public void shouldInitializeLuceneIndex() throws Exception {
		HibernateCallback<?> createLuceneIndexes = executeHibernateCallback();

		MassIndexer massIndexer = mock(MassIndexer.class);

		when(fullTextSession.createIndexer()).thenReturn(massIndexer);
		assertEquals(now, createLuceneIndexes.doInHibernate(session));
	...
	@SuppressWarnings("unchecked")
	@Test
	public void shouldReturnEmptyCollectionIfExceptionSearchingTracks() throws Exception {
		String keyword = "SÓmë Keywörd";
		...
		when(fullTextSessionfactory.createFullTextSession(session)).thenReturn(fullTextSession);
		when(fullTextSession.beginTransaction()).thenReturn(transaction);
		when(fullTextSession.getSearchFactory()).thenThrow(new RuntimeException("some lucene exception"));

		List<MobileTrack> actual = callback.doInHibernate(session);
