...
public class ForwardingService {
	...
	@MessageMethod(ForwardMessage.TYPE)
	public void processForwardMessage(ForwardMessage forwardMessage) {
		AllMessage<?> body = forwardMessage.getBody();
		String forwardTo = forwardMessage.getContactId();
		Set<InetAddress> destinations = (Set<InetAddress>) dhtManager.getOwners(PeerUtils.createPresenceKey(forwardTo));
		if (destinations.isEmpty()) {
			log.warn("COULD NOT FIND ANY ULTRAPEER WITH A SESSION TO " + forwardTo);
		}
		for (InetAddress destination : destinations) {
			forwardTo(body, destination, forwardTo);
		}
	}
	...
	boolean isSandbox() {
		try {
			return peerSettings.getPublicIp().isLoopbackAddress();
		} catch (Exception e) {
			return false;
		}
	}

	private void forwardTo(AllMessage<?> message, InetAddress destination, String forwardTo) {
		message.putProperty(MessEngineConstants.PROP_FORWARD_TO, forwardTo);
		forwardTo(message, destination);
	}
